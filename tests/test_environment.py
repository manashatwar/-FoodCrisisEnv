...
from irce.models import BatchRecord, FoodCrisisAction


def test_reset_returns_observation() -> None:
    env = FoodCrisisEnv(task_id=1, seed=7)
...
def test_quarantine_blocks_spread() -> None:
    ...
    env.step(FoodCrisisAction(action_type=f"QUARANTINE {source}"))
...
def test_inspect_costs_budget_and_returns_result_next_step() -> None:
    ...
    env.step(FoodCrisisAction(action_type=f"INSPECT {target}"))
...
def test_recall_removes_batch() -> None:
    ...
    env.step(FoodCrisisAction(action_type=f"RECALL {batch_id}"))
...
def test_illness_report_delay() -> None:
    env = FoodCrisisEnv(task_id=2, seed=7)
...
def test_grader_is_deterministic() -> None:
    env_one = FoodCrisisEnv(task_id=1, seed=7)
    env_two = FoodCrisisEnv(task_id=1, seed=7)
    env_one.reset(seed=7, task_id=1)
    env_two.reset(seed=7, task_id=1)
    actions = ["MODIFY", "SWITCH", "WAIT", "WAIT"]
    for action in actions:
        env_one.step(FoodCrisisAction(action_type=action))
        env_two.step(FoodCrisisAction(action_type=action))
